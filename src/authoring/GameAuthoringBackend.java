...
import authoring.model.triggers.externaltriggers.SpeedExternalTrigger;
...
public class GameAuthoringBackend {
	...
	public static void main (String args[]) throws GameFileException{
		...
		Property<Integer> toAdd = new Property<Integer>("speed", 25);
		ast.addProperty(toAdd);
		toAdd = new Property<Integer>("speed", 26);
		ast2.addProperty(toAdd);
		
		ArrayList<Actor> list = new ArrayList<Actor>();
		list.add(ast);
		list.add(ast2);
		AExternalTrigger trueExternalTrigger = new TrueExternalTrigger(actionBundle, list);
		AExternalTrigger speedExternalTrigger = new SpeedExternalTrigger(actionBundle, list);

		trueExternalTrigger.condition();
		speedExternalTrigger.condition();
	}
